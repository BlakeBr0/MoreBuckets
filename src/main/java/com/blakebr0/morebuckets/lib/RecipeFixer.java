...
public class RecipeFixer {
	...
	public static void fixRecipes() { // TODO: REMOVE 
		ForgeRegistries.RECIPES.register(new ShapelessRecipes("wow", new ItemStack(Items.ACACIA_BOAT), NonNullList.<Ingredient>withSize(1, Ingredient.fromStacks(new ItemStack(Items.WATER_BUCKET)))).setRegistryName("tester"));
		...
		ForgeRegistries.RECIPES.forEach(recipe -> {
			...
			for (int i = 0; i < ing.size(); i++) {
				ItemStack[] matchingStacks = ing.get(i).getMatchingStacks();
				for (ItemStack stack : matchingStacks) {
					 if (stack.getItem() instanceof UniversalBucket || stack.getItem() == Items.LAVA_BUCKET || stack.getItem() == Items.WATER_BUCKET || stack.getItem() == Items.MILK_BUCKET) {
						 ing.set(i, new FluidIngredient(matchingStacks));
